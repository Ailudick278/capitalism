...
public final class LaborCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> d) {
        ...
        var region = Commands.argument("region", StringArgumentType.word()).executes(c -> post(
    ...
    private static int post(ServerPlayer p, String company, String role, int vacancies, long wage, String skill, int min, int days, String region) { try { boolean ok = LaborMarketService.post(p, company, role, vacancies, wage, LaborSkill.valueOf(skill.toUpperCase()), min, days, region); p.sendSystemMessage(Component.literal(ok ? "job posted region=" + region : "job posting failed")); return ok ? 1 : 0; } catch (IllegalArgumentException e) { p.sendSystemMessage(Component.literal("unknown skill or invalid parameter")); return 0; } }
    private static int jobs(ServerPlayer p) { for (JobOffer o : LaborMarketSavedData.get(p.getServer()).openOffers(p.getServer().overworld().getGameTime())) p.sendSystemMessage(Component.literal(o.id().substring(0, 8) + " employer=" + o.employerId() + " region=" + o.region() + " role=" + o.role() + " vacancies=" + o.vacancies() + " wage=" + o.dailyWageMinor() + " skill=" + o.requiredSkill() + " min=" + o.minimumSkill())); return 1; }
