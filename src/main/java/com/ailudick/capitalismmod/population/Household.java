...
public record Household(String id, String region, int size, int workingAge,
    ...
    public Household(String id, String region, int size, int workingAge, long cashMinor,
        ...
        this(id, region, size, workingAge, cashMinor, dailyNeedMinor, satisfaction, lastSettlementDay, -1L, 70, 0, 0);
    }
    public Household(String id, String region, int size, int workingAge, long cashMinor,
                     long dailyNeedMinor, int satisfaction, long lastSettlementDay, long lastMigrationDay) {
        this(id, region, size, workingAge, cashMinor, dailyNeedMinor, satisfaction, lastSettlementDay, lastMigrationDay, 70, 0, 0);
    }
    public Household(String id, String region, int size, int workingAge, long cashMinor,
                     long dailyNeedMinor, int satisfaction, long lastSettlementDay,
    ...
    public Household withSettlement(long day, long cash, int satisfaction, String nextRegion) {
        return new Household(id, nextRegion, size, workingAge, Math.max(0L, cash), dailyNeedMinor,
                Math.max(0, Math.min(100, satisfaction)), day, lastMigrationDay, health, education, unemploymentDays);
    }
    public Household withCash(long cash) { return new Household(id, region, size, workingAge, Math.max(0L, cash), dailyNeedMinor, satisfaction, lastSettlementDay, lastMigrationDay, health, education, unemploymentDays); }
    public Household withRegion(String nextRegion, long day) { return new Household(id, nextRegion, size, workingAge, cashMinor, dailyNeedMinor, satisfaction, lastSettlementDay, day, health, education, unemploymentDays); }
    ...
    public Household withEmploymentState(boolean employed) {
        ...
        return new Household(id, region, size, workingAge, cashMinor, dailyNeedMinor, satisfaction,
                lastSettlementDay, lastMigrationDay, health, education, days);
    }
}
