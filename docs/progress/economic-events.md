# 动态经济事件系统开发进度

## 已实现功能

- 使用统一事件对象记录事件 ID、系统、类型、起止时间、目标主体、幅度和状态。
- 商品价格冲击支持正向和负向基点幅度，按持续时间传导到商品价格更新。
- 事件到期后自动标记为 `resolved`，重复创建同一事件 ID 不会重复生效。
- 商品价格更新和每日结算都会检查到期事件，因此市场暂停时事件状态仍会随模拟时间恢复。
- `/economicevent priceShock <eventId> <itemId> <shockBps> <days>` 可由管理员创建可复现的市场冲击。
- `shockBps` 为基点：正值表示供给收缩/价格上行，负值表示需求收缩/价格下行；例如 `2500` 代表约 25%。
- `/economicevent list` 以只读方式查看当前有效事件、目标商品和剩余模拟时间。
- `/economicevent logisticsShock <eventId> <origin> <destination> <capacityBps> <days>` 会改变指定路线的批次运力和运输时间；负值表示中断/拥堵，正值表示临时增容。
- 路线冲击还会按同一基点幅度调整运输中断概率，并限制在 0% 到 95% 之间；因此物流损失、保险索赔和合同违约可以沿现有结算链继续发生。
- `/economicevent laborShock <eventId> <region> <demandBps> <days>` 会按区域调整当日可吸收的招聘额度；已有就业合同不受追溯修改，负冲击会通过未就业家庭进入消费和迁移系统。
- 当价格达到基础价值的 2.5 倍或低于 40% 时，市场会自动生成 3 天、±1000 基点的反向稳定事件；同一商品已有冲击时不会叠加新事件。
- 系统性金融危机进入和恢复会自动写入/结束 `financial_crisis` 事件，并与现有授信冻结状态保持同一 episode。

## 存在的问题

- 自动稳定器和管理员事件已覆盖商品价格、路线物流和区域招聘额度；金融危机已能自动生成事件，但灾害和环境状态仍未接入事件生成。
- 物流事件已经可以通过运输损失、保险索赔和合同违约进入企业账本；就业冲击可以通过家庭失业状态继续影响消费和迁移，财政救济与金融风险的专门事件传导仍待完善。
- 多个同时发生的事件目前按基点简单相加，尚未实现传播网络和事件优先级。

## 后续方向

1. 增加固定种子和事件模板，让供给中断、需求骤降、自然灾害和金融危机可重复模拟。
2. 建立区域传播、持续期和恢复任务，使事件影响物流、就业、价格和迁移。
3. 将事件与保险、政府救济、企业现金流和金融风险状态机统一连接。
